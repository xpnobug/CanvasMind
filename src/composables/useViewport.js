...
export function useViewport(options = {}) {
   ...
   */
  function clampViewport(containerRect, contentWidth, contentHeight, margin = 200) {
   ...
   * 以指定点为中心缩放
   */
  function zoomAt(clientX, clientY, delta, containerRect) {
   ...
   * 设置缩放（以视口中心为基准）
   */
  function setZoom(newZoom, containerRect) {
   ...
   * 居中显示内容
   */
  function centerContent(contentWidth, contentHeight, containerRect) {
   ...
   * 获取视口边界（画布坐标系）
   */
  function getViewportBounds(containerRect) {
